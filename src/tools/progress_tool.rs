...
impl Tool for ProgressTool {
    ...
    async fn execute(&self, _ctx: Arc<dyn ToolContext>, args: Value) -> AdkResult<Value> {
        let operation = args["operation"]
            .as_str()
            .ok_or_else(|| adk_rust::AdkError::Tool("Missing 'operation' field".to_string()))?;
        ...
        match operation {
            "read" => self.read().await.map_err(adk_rust::AdkError::Tool),
            "append" => {
                let entry: ProgressEntryInput = serde_json::from_value(args["entry"].clone())
                    .map_err(|e| adk_rust::AdkError::Tool(format!("Invalid entry: {}", e)))?;
                self.append(entry).await.map_err(adk_rust::AdkError::Tool)
            }
            "summary" => {
