...
impl Tool for FileTool {
    ...
    async fn execute(&self, _ctx: Arc<dyn ToolContext>, args: Value) -> Result<Value> {
        ...
        match args.operation.as_str() {
            ...
            "write" => {
                ...
                if let Some(parent) = full_path.parent() {
                    ...
                    std::fs::create_dir_all(parent).map_err(|e| {
                        adk_rust::AdkError::Tool(format!("Failed to create directories: {}", e))
                    })?;
                }
            ...
            "delete" => {
                ...
                if full_path.is_dir() {
                    std::fs::remove_dir_all(&full_path).map_err(|e| {
                        adk_rust::AdkError::Tool(format!("Failed to delete directory: {}", e))
                    })?;
                } else {
                    std::fs::remove_file(&full_path).map_err(|e| {
                        adk_rust::AdkError::Tool(format!("Failed to delete file: {}", e))
                    })?;
                }
            ...
            op => Err(adk_rust::AdkError::Tool(format!(
...
impl Tool for ListFilesTool {
    ...
    async fn execute(&self, _ctx: Arc<dyn ToolContext>, args: Value) -> Result<Value> {
        ...
        let entries: Vec<Value> = std::fs::read_dir(&full_path)
            .map_err(|e| adk_rust::AdkError::Tool(format!("Failed to read directory: {}", e)))?
            .filter_map(|entry| entry.ok())
            .map(|entry| {
                let name = entry.file_name().to_string_lossy().to_string();
                let is_dir = entry.file_type().map(|t| t.is_dir()).unwrap_or(false);
