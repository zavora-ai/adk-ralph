...
fn init_telemetry(config: &TelemetryConfig, debug_level: DebugLevel) -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref endpoint) = config.otlp_endpoint {
        ...
        adk_rust::telemetry::init_with_otlp(&config.service_name, endpoint)?;
    } else {
        // Initialize basic telemetry with console logging
        adk_rust::telemetry::init_telemetry(&config.service_name)?;
    }
...
async fn main() -> Result<()> {
    ...
    adk_rust::telemetry::shutdown_telemetry();
